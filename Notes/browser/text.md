...
- [图解浏览器渲染过程](#图解浏览器渲染过程)
  - [1.Chrome浏览器多进程/线程模型](#1chrome浏览器多进程线程模型)
      ...
      - [3.渲染过程](#3渲染过程)
        ...
        - [3.2 RenderObject](#32-renderobject)
        - [3.3 RenderLayer与渲染绘制](#33-renderlayer与渲染绘制)
    - [浏览器渲染原理及流程](#浏览器渲染原理及流程)
      - [1.浏览器主要组件结构](#1浏览器主要组件结构)
      - [2.渲染主流程](#2渲染主流程)
        - [2.1 关键渲染路径](#21-关键渲染路径)
        - [2.2 构建DOM树](#22-构建dom树)
        - [2.3 构建CSSOM规则树](#23-构建cssom规则树)
        - [2.4 渲染阻塞](#24-渲染阻塞)
        - [2.5 构建渲染树](#25-构建渲染树)
        - [2.6 渲染树布局](#26-渲染树布局)
        - [2.7 渲染树绘制](#27-渲染树绘制)
        - [2.8 减少reflow/repaint](#28-减少reflowrepaint)
      - [3.刷新率](#3刷新率)
      - [5.window.requestAnimationFrame()](#5windowrequestanimationframe)
      - [5.window.requestIdleCallback()](#5windowrequestidlecallback)
        - [5.1 timeRemaining() 方法](#51-timeremaining-方法)
        - [5.2 didTimeout属性](#52-didtimeout属性)
      - [4.页面渲染优化](#4页面渲染优化)
      - [参考：](#参考)
    - [浏览器输入URL后发生了什么](#浏览器输入url后发生了什么)
      ...
      - [参考：](#参考-1)
    - [DNS解析](#dns解析)
      - [1.为什么需要DNS解析域名为IP地址？](#1为什么需要dns解析域名为ip地址)
      - [2.具体什么是DNS？](#2具体什么是dns)
      - [3.DNS 的过程？](#3dns-的过程)
      - [4.DNS服务的体系架构是怎样的？](#4dns服务的体系架构是怎样的)
        - [4.1 DNS域名称](#41-dns域名称)
        - [4.2 DNS 域的名称层次结构](#42-dns-域的名称层次结构)
        - [4.3 DNS域名称空间的组织方式](#43-dns域名称空间的组织方式)
        - [4.4 DNS 和 Internet 域](#44-dns-和-internet-域)
        - [4.5 资源记录](#45-资源记录)
        - [4.6 Dns服务的工作过程当](#46-dns服务的工作过程当)
      - [参考：](#参考-2)
