/*
* 自平衡数树 -- AVLTree
* BST 存在一个问题：取决于添加的节点数，
* 可能树的一条边会非常深。
* 这会在某条边上操作时引起一些性能问题。
* 为了解决这个问题，
* 引入 Adelson-Velskii-Landi 数 (AVL 树)，
* 是一种自平衡二叉搜索树，
* 即任何一个节点左右两侧子树的高度只差最多为 1。
* 也就是说这种树会在添加或移除节点时尽量试着成为一颗完全树。
*
*/
...
// 在 AVL 树中插入或移除节点和 BST 完全相同，只是 AVL 树要检验“平衡因子”
 ...
 *  计算平衡因子
 ******************* */
// 在 AVL 树中，需要对每个节点计算右子树高度(hr)
// 和左子树的高度(hl)的差值，
// 该值（hr-hl）应为 0/1 或 -1，否则需要平衡树
