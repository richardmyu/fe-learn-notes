function ArrayList() {
   ...
   *    冒泡排序    *
   ***************** */
  //比较任意相邻的项，如果第一项大于第二项则交换位置，较大的项依次沉到底部，而小的项逐渐冒出
   ...
   *    插入排序    *
   ***************** */
  //指定某一项为参考项，依次比较该项之前的其他项与参考项的大小，从而决定其他项的插入的位置
  //排序小型数组时，性能好于选择排序和冒泡排序
  ...
  //前三个性能不好，归并排序性能好，其复杂度为O(nlogn)
  //归并排序是一种分治算法。其思想是将原始数组切分成较小的数组，直到每一个小数组只有一个位置，接着将小数组归并为较大的数组，直到最后只有一个排序完毕的大数组
  ...
  //最常用的排序算法，复杂度为O(nlogn),且它的性能通常比其他的同等复杂度的排序算法要好
  //也是采用分治的方法
  //1.从数组中选取中间一项作为主元
  //2.创建两个指针，左边一个指向数组的第一项，右边的一个指向数组的最后一项；
  //   移动左指针直到找到比主元大的元素；
  //   再移动右指针直到找到比主元小的元素；
  //   交换
  //   重复以上，直到左指针超过右指针
  //3.算法对划分后的小组（较主元小的子数组和较主元大的子数组）重复之前的两步，直至数组完全排序
